...
# bad - wastes horizontal space, messier diffs when refactoring
kind = case year
       when 1850..1889 then 'Blues'
       when 1890..1909 then 'Ragtime'
       when 1910..1929 then 'New Orleans Jazz'
...
# okay too, if the line is too long using the above style
result =
  if some_cond
    calc_something
  else
...
Avoid comma after the last parameter in a method call, unless the parameters are on separate lines.
...
# good (for long lines0) - easier to move/add/remove parameters
some_method(
  size,
  count,
  color,
...
For function calls that do not fit on one line, use single indent for the lines after the first.
...
Align the elements of array literals spanning multiple lines on a new indent level.
...
Separate numbers from letters on symbols, methods and variables.
...
# good - preferred when v is only used within block
if (v = array.grep(/foo/))
  do_something(v)
  # some code
end
...
Prefer `fail` when raising a new error; `raise` when re-raising in a `rescue` block.
...
Avoid comma after the last item of an `Array` or `Hash` literal, except when the items are on separate lines.
...
Use `Hash#each_key` instead of `Hash#keys.each` and `Hash#each_value` instead of `Hash#values.each`, unless you need to iterate over key-value pairs.
